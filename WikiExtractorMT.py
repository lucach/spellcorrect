    ...
    def _get_file(cls, outputdir, compress=False):
        with cls._filename_lock:
            ...
            while not fpath or os.path.exists(fpath):
                fname = ''.join([random.choice(string.letters)
                                 for _ in range(16)])
    ...
    def _write(self, wiki_id, wiki_title, wiki_text):
        ...
        if colon < 0 or wiki_title[:colon] in acceptedNamespaces:
            ...
            if self._output_format == TANL:
                ...
                article = dict(
                    id=wiki_id, url=url, title=wiki_title, text=wiki_text)
                self._outfile.write(
...
# Template = "{{" [ "msg:" | "msgnw:" ] PageName { "|"
#   [ ParameterName "=" AnyText | AnyText ] } "}}" ;
...
#
# ParameterName = ? uppercase, lowercase, numbers,
#   no spaces, some special chars ? ;
#
# ===========================================================================
...
for tag in selfClosingTags:
    pattern = re.compile(
        r'<\s*%s\b[^/]*/\s*>' % tag, re.DOTALL | re.IGNORECASE)
...
def main():
    ...
    parser.add_argument("-b", "--bytes", default="25M",
                        help="put specified bytes per output file " +
                        "(default is %(default)s)", metavar="n[KM]")
    parser.add_argument("-c", "--compress", default=False,
                        action="store_const", const=True,
                        help="compress output files using bzip")
    parser.add_argument("-l", "--links", default=False,
                        action="store_const", const=True,
                        help="preserve links")
    parser.add_argument("-s", "--sections", default=False,
                        action="store_const", const=True,
